...
const DataTransaksiSelesai = () => {
    ...
    useEffect(() => {
        const fetchTransaksi = async () => {
            try {
                setLoading(true);
                const res = await API.get("/transaksi/status/Selesai");
    ...
    const handleDiambil = async (id) => {
        if (!window.confirm("Apakah Anda yakin ingin menandai transaksi ini sebagai diambil?")) return;
        try {
            const res = await API.patch(`/transaksi/${id}`, { status: "Diambil" });
            if (res.ok) {
                ...
                <div className="container mx-auto px-4 py-8 max-w-7xl">
                    ...
                    <div className="bg-white rounded-xl shadow-lg border border-gray-100 overflow-hidden">
                        <div className="bg-gradient-to-r from-emerald-500 to-green-600 p-6">
                            ...
                            <p className="text-emerald-100 text-sm mt-1">Memuat data transaksi...</p>
                        </div>
                        <div className="p-6">
                ...
                <div className="bg-white rounded-xl shadow-lg border border-gray-100 overflow-hidden">
                        ...
                        <div className="flex justify-between items-center">
                            <div>
                                ...
                                <p className="text-emerald-100 text-sm mt-1">
                                    {transaksi.length > 0 ? `${transaksi.length} transaksi siap diambil` : 'Tidak ada transaksi selesai'}
                                </p>
                            </div>
                            <div className="text-right">
                                <div className="bg-white/20 backdrop-blur-sm rounded-lg px-4 py-2">
                                    <span className="text-white text-sm font-medium">Status: Selesai</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    {transaksi.length === 0 ? (
                        <div className="p-12 text-center">
                            ...
                            <h4 className="text-lg font-semibold text-gray-600 mb-2">Semua Sudah Diambil!</h4>
                            <p className="text-gray-500 mb-6">Tidak ada transaksi selesai yang menunggu untuk diambil</p>
                            <Link to="/petugas/transaksi">
                                <button className="bg-emerald-500 text-white px-6 py-3 rounded-lg hover:bg-emerald-600 transition-colors duration-200">
                                    Lihat Transaksi Proses
                                </button>
                            </Link>
                        </div>
                    ) : (
                                        ...
                                        <tr key={t.id_transaksi} className="hover:bg-gray-50 transition-colors duration-200">
                                            ...
                                            <td className="py-4 px-6">
                                                <div className="flex items-center space-x-2">
                                                    ...
                                                    <button 
                                                        onClick={() => handleDiambil(t.id_transaksi)}
                                                        className="bg-purple-100 text-purple-600 px-3 py-1 rounded-lg hover:bg-purple-200 transition-colors duration-200 text-sm font-medium flex items-center space-x-1"
                                                    >
                                                        <span>📦</span>
                                                        <span>Diambil</span>
                                                    </button>
                        ...
                        <div className="bg-gray-50 px-6 py-4 border-t border-gray-200">
                            <div className="flex items-center justify-between">
                                <p className="text-sm text-gray-600">
                                    {transaksi.length} transaksi siap diambil
                                </p>
                                <div className="flex items-center space-x-2">
                                    <div className="w-2 h-2 bg-green-400 rounded-full"></div>
                                    <span className="text-xs text-gray-500">Status: Selesai</span>
                                </div>
                            </div>
                        </div>
                    )}
...
export default DataTransaksiSelesai;
